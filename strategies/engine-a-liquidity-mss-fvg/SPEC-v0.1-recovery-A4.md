# Engine A Original Recovery — Variant A4

**Status:** FROZEN BEFORE OUTCOME INSPECTION  
**Version:** 0.1-recovery-A4  
**Frozen:** 2026-09-22  
**Experiment:** EXP-014 Part A

## Purpose

Variant A4 is the fourth fully mechanical attempt to reconstruct the original EXP-002 XAUUSD Engine A. It inherits A3 mechanics and changes exactly one ambiguity: 5m liquidity-swing confirmation strength.

It is a recovery hypothesis, not a claim that these missing details were the unpublished original implementation. No A4 parameter may be changed after A4 outcomes are calculated. All A3 rules remain unchanged except the 5m liquidity-swing definition below.
...
## 5m liquidity swing — A4 single-ambiguity change

Use a more selective **2-left / 2-right** confirmed 5m pivot.
...
The pivot becomes eligible only after t+2 has completed, preventing future-looking use.
...
No other liquidity lookback/reuse rule changes in A4.
...
Maximum favorable excursion is measured over the **actual A4 trade life**: from entry until T5, structural stop, or the frozen time horizon, whichever occurs first. Because fill-bar ordering is unknowable, A4 records fill-bar MFE as zero and begins MFE accumulation with the next completed 1m bar. For a T5 winner, realized MFE is capped at USD 5 because the trade exits at T5.
...
For the EXP-002 reproduction comparison, A4 simplified T5 P&L in Gold-price units is:
...
Compare A4 to the frozen numerical acceptance protocol in:

- research/experiments/EXP-014-original-engine-a-recovery-equivalent-sizing.md.

A4 is a recovered reproduction only if the full frozen PASS definition is satisfied.
