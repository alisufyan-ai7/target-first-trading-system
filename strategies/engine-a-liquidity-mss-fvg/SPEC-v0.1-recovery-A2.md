# Engine A Original Recovery — Variant A2

**Status:** FROZEN BEFORE OUTCOME INSPECTION  
**Version:** 0.1-recovery-A2  
**Frozen:** 2026-09-22  
**Experiment:** EXP-014 Part A

## Purpose

Variant A2 is the second fully mechanical attempt to reconstruct the original EXP-002 XAUUSD Engine A. It changes exactly one A1 ambiguity: MSS and displacement may occur on adjacent candles rather than being forced onto the same candle.

It is a recovery hypothesis, not a claim that these missing details were the unpublished original implementation. No A2 parameter may be changed after A2 outcomes are calculated. All rules not explicitly changed below are inherited unchanged from A1.
...
A2 uses FVG invalidation rather than the distant 5m sweep extreme.
...
Maximum favorable excursion is measured over the **actual A2 trade life**: from entry until T5, structural stop, or the frozen time horizon, whichever occurs first. Because fill-bar ordering is unknowable, A2 records fill-bar MFE as zero and begins MFE accumulation with the next completed 1m bar. For a T5 winner, realized MFE is capped at USD 5 because the trade exits at T5.
...
For the EXP-002 reproduction comparison, A2 simplified T5 P&L in Gold-price units is:
...
Compare A2 to the frozen numerical acceptance protocol in:

- research/experiments/EXP-014-original-engine-a-recovery-equivalent-sizing.md.

A2 is a recovered reproduction only if the full frozen PASS definition is satisfied.
