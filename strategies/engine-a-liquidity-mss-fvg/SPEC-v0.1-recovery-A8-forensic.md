# Engine A Original Recovery — Variant A8 Forensic Timing Diagnostic

**Status:** FROZEN BEFORE OUTCOME INSPECTION — FORENSIC / NON-CAUSAL; NOT DEPLOYABLE  
**Version:** 0.1-recovery-A8-forensic  
**Frozen:** 2026-09-22  
**Experiment:** EXP-014 Part A

## Purpose

Variant A8 is a forensic sibling of A6. It tests one unresolved ambiguity: whether the original exploratory code effectively allowed the 1m MSS/displacement sequence to begin inside a 5m bar that was only later known to have completed as a valid sweep/rejection bar. This intentionally tests a possible look-ahead mechanism; it is not an admissible live engine.

It is a recovery hypothesis, not a claim that these missing details were the unpublished original implementation. No A8 parameter may be changed after A8 outcomes are calculated. All A6 rules remain unchanged except the MSS timing/anchor described below.
...
## 1m internal pivot for MSS — A8 forensic timing change

For A8 only, anchor the internal pivot to information available **before the sweep breach occurs**, rather than waiting for the 5m sweep bar to complete.

Within the 5m bar that is later classified as a valid sweep/rejection:

1. identify the first constituent 1m bar that breaches the active 5m liquidity level;
2. immediately before that breach bar, select the most recent already-confirmed opposite-side 1m pivot from the preceding 12 completed minutes.

The 1m pivot definition remains unchanged:

Confirmed swing low at minute t:

- low[t] < low[t-1];
- low[t] <= low[t+1].

Confirmed swing high is mirrored.

The pivot is eligible only after t+1 has completed.

If no such pivot exists before the first breach, the setup expires.

## MSS timing and rule — A8 forensic timing change

Starting with the **first constituent 1m bar at or after the liquidity breach inside the sweep 5m bar**, search at most 10 completed 1m bars for the first MSS close.

Bearish MSS:

- 1m close < the frozen pre-breach internal swing low.

Bullish MSS:

- 1m close > the frozen pre-breach internal swing high.

Only the first qualifying MSS is used.

Important forensic limitation:

- the containing 5m bar is classified as a valid sweep only because its eventual 5m close returns inside the liquidity level;
- an A8 MSS/FVG/entry may therefore occur before that 5m close is known;
- this is deliberate look-ahead for recovery diagnosis and is **not permitted for a production strategy**.
...
Maximum favorable excursion is measured over the **actual A8 forensic trade life**: from entry until T5, structural stop, or the frozen time horizon, whichever occurs first. Because fill-bar ordering is unknowable, A8 records fill-bar MFE as zero and begins MFE accumulation with the next completed 1m bar. For a T5 winner, realized MFE is capped at USD 5 because the trade exits at T5.
...
For the EXP-002 reproduction comparison, A8 simplified T5 P&L in Gold-price units is:
...
Compare A8 to the frozen numerical acceptance protocol in:

- research/experiments/EXP-014-original-engine-a-recovery-equivalent-sizing.md.

A8 can reproduce the historical benchmark only if the full frozen PASS definition is satisfied.
