# Engine A Original Recovery — Variant A6

**Status:** FROZEN BEFORE OUTCOME INSPECTION  
**Version:** 0.1-recovery-A6  
**Frozen:** 2026-09-22  
**Experiment:** EXP-014 Part A

## Purpose

Variant A6 is a sibling recovery branch from the A3 baseline. A4 and A5 pivot-strength branches were rejected. A6 changes exactly one ambiguity: whether the latest eligible 5m liquidity level is permanently consumed after its first qualifying sweep.

It is a recovery hypothesis, not a claim that these missing details were the unpublished original implementation. No A6 parameter may be changed after A6 outcomes are calculated. All A3 rules remain unchanged except liquidity-level reuse below.
...
Maximum favorable excursion is measured over the **actual A6 trade life**: from entry until T5, structural stop, or the frozen time horizon, whichever occurs first. Because fill-bar ordering is unknowable, A6 records fill-bar MFE as zero and begins MFE accumulation with the next completed 1m bar. For a T5 winner, realized MFE is capped at USD 5 because the trade exits at T5.
...
For the EXP-002 reproduction comparison, A6 simplified T5 P&L in Gold-price units is:
...
Compare A6 to the frozen numerical acceptance protocol in:

- research/experiments/EXP-014-original-engine-a-recovery-equivalent-sizing.md.

A6 is a recovered reproduction only if the full frozen PASS definition is satisfied.
