# Engine A Original Recovery — Variant A3

**Status:** FROZEN BEFORE OUTCOME INSPECTION  
**Version:** 0.1-recovery-A3  
**Frozen:** 2026-09-22  
**Experiment:** EXP-014 Part A

## Purpose

Variant A3 is the third fully mechanical attempt to reconstruct the original EXP-002 XAUUSD Engine A. It inherits A2 candidate generation unchanged and changes exactly one ambiguity: the FVG-stop buffer.

It is a recovery hypothesis, not a claim that these missing details were the unpublished original implementation. No A3 parameter may be changed after A3 outcomes are calculated. All candidate-generation, session, entry, horizon, and outcome rules are inherited unchanged from A2.
...
Maximum favorable excursion is measured over the **actual A3 trade life**: from entry until T5, structural stop, or the frozen time horizon, whichever occurs first. Because fill-bar ordering is unknowable, A3 records fill-bar MFE as zero and begins MFE accumulation with the next completed 1m bar. For a T5 winner, realized MFE is capped at USD 5 because the trade exits at T5.
...
For the EXP-002 reproduction comparison, A3 simplified T5 P&L in Gold-price units is:
...
Compare A3 to the frozen numerical acceptance protocol in:

- research/experiments/EXP-014-original-engine-a-recovery-equivalent-sizing.md.

A3 is a recovered reproduction only if the full frozen PASS definition is satisfied.
