#!/usr/bin/env python3
"""EXP-029 Engine M v0.3 zero-outcome selective preflight."""
...
from engine_m_v0_3 import selective_setup_at, self_tests_engine_m_v03
...
CACHE=Path("/tmp/engine-m-v03-preflight")
...
def synthetic_selective_setup_tests()->list[str]:
    tests=[]

    # Long: HTF up, prior-4 M15 low swept/reclaimed, H1 midpoint reclaimed,
    # M5 outer-quartile arm, and recent H1 high beyond T40.
    h4=pd.DataFrame({
        ...
        "open":[1.10,1.10],"high":[1.20,1.30],"low":[1.00,1.05],"close":[1.12,1.22],
    })
    h1=pd.DataFrame({
        ...
        "open":[1.12,1.16],"high":[1.30,1.36],"low":[1.08,1.10],"close":[1.15,1.24],
    })
    m15=pd.DataFrame({
        "datetime":pd.to_datetime([
            "2026-04-06T07:00Z","2026-04-06T07:15Z","2026-04-06T07:30Z",
            "2026-04-06T07:45Z","2026-04-06T08:00Z"
        ]),
        "available_ts":pd.to_datetime([
            "2026-04-06T07:15Z","2026-04-06T07:30Z","2026-04-06T07:45Z",
            "2026-04-06T08:00Z","2026-04-06T08:15Z"
        ]),
    ...
    m5=pd.DataFrame({
        ...
        "open":[1.17,1.18],"high":[1.20,1.25],"low":[1.16,1.15],"close":[1.18,1.24],
    })
    x=selective_setup_at({"h4":h4,"h1":h1,"m15":m15,"m5":m5},4,"EURUSD")
    assert x["status"]=="armed" and x["direction"]=="long"
    assert x["s15_low"]<x["prev4_low"]
    assert x["s15_close"]>x["prev4_low"] and x["s15_close"]>x["h1_mid"]
    assert x["h1_destination"]>=x["target_price"]
    assert x["limit"]<x["a5_close"]
    tests.append("synthetic_long_liquidity_reclaim_target_room")

    # Equality at prior low is not a sweep.
    m15eq=m15.copy()
    m15eq.loc[4,"low"]=float(m15eq.iloc[:4]["low"].min())
    y=selective_setup_at({"h4":h4,"h1":h1,"m15":m15eq,"m5":m5},4,"EURUSD")
    assert y["status"]=="m15_liquidity_sweep_failed"
    tests.append("strict_equal_low_not_sweep")
    ...
    m15s.loc[2,["open","high","low","close"]]=[1.06,1.12,1.02,1.05]
    m15s.loc[3,["open","high","low","close"]]=[1.05,1.11,1.01,1.06]
    m15s.loc[4,["open","high","low","close"]]=[1.07,1.13,0.91,1.00]
    m5s=m5.copy()
    m5s.loc[0,["open","high","low","close"]]=[1.07,1.09,1.02,1.06]
    m5s.loc[1,["open","high","low","close"]]=[1.06,1.12,0.92,0.94]
    z=selective_setup_at({"h4":h4s,"h1":h1s,"m15":m15s,"m5":m5s},4,"EURUSD")
    assert z["status"]=="armed" and z["direction"]=="short"
    assert z["s15_high"]>z["prev4_high"]
    assert z["s15_close"]<z["prev4_high"] and z["s15_close"]<z["h1_mid"]
    assert z["h1_destination"]<=z["target_price"]
    assert z["limit"]>z["a5_close"]
    tests.append("synthetic_short_liquidity_reclaim_target_room")
...
def market_preflight(symbol:str,df1:pd.DataFrame,usd_jpy_df:pd.DataFrame|None=None)->dict:
    ...
    for i in range(len(bars["m15"])):
        ...
        setup=selective_setup_at(bars,i,symbol)
        ...
        if len(examples)<5:
            examples.append({
                ...
                "direction":direction,
                "prev4_low":setup["prev4_low"],
                "prev4_high":setup["prev4_high"],
                "h1_mid":setup["h1_mid"],
                "h1_destination":setup["h1_destination"],
...
def main():
    tests={
        ...
        "engine_m_v03":self_tests_engine_m_v03(),
        "synthetic_selective_setup":synthetic_selective_setup_tests(),
    ...
    per_market_gate={
        s:(
            markets[s]["filled_signal_paths"]>=35
    ...
    gate={
        "each_market_ge_35_and_both_directions":all(per_market_gate.values()),
        "total_signal_paths_ge_400":total_signals>=400,
    ...
    passed=bool(
        gate["each_market_ge_35_and_both_directions"]
        and gate["total_signal_paths_ge_400"]
    ...
    result={
        "experiment":"EXP-029",
        "engine":"Engine M v0.3",
        "stage":"zero_outcome_selective_liquidity_reclaim_preflight",
        "target_outcomes_calculated":False,
        "pnl_outcomes_calculated":False,
    ...
    out=OUT/"EXP-029-selective-preflight-v0.3.json"
