...
require("./engine-g-v0.1.js");
const Engine = globalThis.EngineGv01;
if (!Engine) throw new Error("EngineGv01 not loaded");
Engine.selfTest();
...
const dataDir=path.resolve("data/engine-g");
const months=[];
for (const [name,sha,size,expectedRows] of manifest) {
  ...
  if (got!==sha) throw new Error("blob_sha_mismatch:"+name+":"+got);
  months.push({name,content:buf.toString("utf8"),expectedRows});
}

const metrics=Engine.runEngineG(months,Date.UTC(2024,0,1),Date.UTC(2025,2,1));
const bootstrap=Engine.bootstrap(metrics,10000,16016);

const resultDir=path.resolve("research/results");
fs.mkdirSync(resultDir,{recursive:true});

const summary={...metrics,bootstrap};
delete summary.trades;
delete summary.setups;
delete summary.daily_series;
summary.execution_metadata={
  engine_implementation:"research/code/engine-g-v0.1.js",
...
fs.writeFileSync(path.join(resultDir,"EXP-016-development-summary-v0.1.json"),JSON.stringify(summary,null,2)+"\n");
fs.writeFileSync(path.join(resultDir,"EXP-016-development-setups-v0.1.jsonl"),metrics.setups.map(x=>JSON.stringify(x)).join("\n")+"\n");
fs.writeFileSync(path.join(resultDir,"EXP-016-development-trades-v0.1.jsonl"),metrics.trades.map(x=>JSON.stringify(x)).join("\n")+"\n");

console.log("EXP016_DEVELOPMENT_SUMMARY",JSON.stringify({
  ...
  s1_hit_rate:metrics.s1_hit_rate,
