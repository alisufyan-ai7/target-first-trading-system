#!/usr/bin/env python3
"""EXP-032 Engine N v0.1 zero-outcome opening-drive preflight."""
...
CACHE=Path("/tmp/engine-n-v01-preflight")
...
def synthetic_engine_n_tests()->list[str]:
    tests=[]
    anchor=pd.Timestamp("2026-04-06T07:00:00Z")

    # Exact eight-bar baseline, each range 0.0018.
    starts=[anchor-pd.Timedelta(minutes=120-15*i) for i in range(8)]
    m15=pd.DataFrame({
        "datetime":starts,
        "available_ts":[x+pd.Timedelta(minutes=15) for x in starts],
        "open":[1.1000]*8,
        "high":[1.1009]*8,
        "low":[1.0991]*8,
        "close":[1.1000]*8,
    ...
    times=[anchor+pd.Timedelta(minutes=i) for i in range(30)]
    rows=[]
    for i,t in enumerate(times):
        frac=i/29
        op=1.1000 if i==0 else 1.1000+0.0028*max(0,(i-1)/29)
        cl=1.1000+0.0030*frac
        hi=max(op,cl)+0.0002
        lo=min(op,cl)-0.0002
        rows.append((t,op,hi,lo,cl))
    df=pd.DataFrame(rows,columns=["datetime","open","high","low","close"])

    x=session_setup(df,m15,anchor,"LONDON","EURUSD")
    assert x["status"]=="armed" and x["direction"]=="long"
    assert x["drive_range_over_baseline_median"]>=1.5
    assert x["drive_body_fraction"]>=0.6
    assert x["limit"]<x["drive_close"]
    tests.append("synthetic_long_drive_qualifies")

    # Add post-drive bars and touch the midpoint on bar 3.
    future=[]
    for i in range(45):
        t=anchor+pd.Timedelta(minutes=30+i)
        if i<2:
            future.append((t,1.1029,1.1031,1.1024,1.1027))
        elif i==2:
            future.append((t,1.1026,1.1028,float(x["limit"])-0.00001,1.1020))
        else:
            future.append((t,1.1020,1.1022,1.1018,1.1020))
    full=pd.concat([
        df,
        pd.DataFrame(future,columns=["datetime","open","high","low","close"])
    ],ignore_index=True)
    fill=find_session_pullback_fill(full,x)
    assert fill["filled"] and fill["wait_active_m1_bars"]==3
    assert abs(fill["entry"]-x["limit"])<1e-12
    tests.append("pullback_limit_fill_exact")

    # Short mirror.
    srows=[]
    for i,t in enumerate(times):
        frac=i/29
        op=1.1030 if i==0 else 1.1030-0.0028*max(0,(i-1)/29)
        cl=1.1030-0.0030*frac
        hi=max(op,cl)+0.0002
        lo=min(op,cl)-0.0002
        srows.append((t,op,hi,lo,cl))
    sdf=pd.DataFrame(srows,columns=["datetime","open","high","low","close"])
    y=session_setup(sdf,m15,anchor,"LONDON","EURUSD")
    assert y["status"]=="armed" and y["direction"]=="short"
    assert y["limit"]>y["drive_close"]
    tests.append("synthetic_short_drive_qualifies")

    # Incomplete drive rejected.
    z=session_setup(df.iloc[:-1].copy(),m15,anchor,"LONDON","EURUSD")
    assert z["status"]=="drive_30m_incomplete"
    tests.append("incomplete_drive_rejected")
...
def market_preflight(symbol:str,df1:pd.DataFrame,usd_jpy_df:pd.DataFrame|None=None)->dict:
    ...
    for day in pd.date_range(START.normalize(),SEALED_START-pd.Timedelta(days=1),freq="D",tz="UTC"):
        if day.weekday()>=5:
            continue
        for hour,minute,name in SESSION_ANCHORS:
    ...
    return {
        ...
        "armed_by_session":dict(sorted(session_armed.items())),
...
def main():
    tests={
        "engine_n_v01":self_tests_engine_n_v01(),
    ...
    result={
        "experiment":"EXP-032",
        "engine":"Engine N v0.1",
        "stage":"zero_outcome_session_opening_drive_preflight",
        "target_outcomes_calculated":False,
        "pnl_outcomes_calculated":False,
    ...
    out=OUT/"EXP-032-opening-drive-preflight-v0.1.json"
