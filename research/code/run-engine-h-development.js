...
require("./engine-h-v0.1.js");
const E=globalThis.EngineHv01;
if(!E) throw new Error("EngineHv01 missing");
E.selfTest();
...
function gitBlobSha(buf){const h=Buffer.from("blob "+buf.length+"\0","utf8");return crypto.createHash("sha1").update(Buffer.concat([h,buf])).digest("hex");}
const dir=path.resolve("data/engine-h"),months=[];
for(const [name,sha,size,expectedRows] of manifest){
  const p=path.join(dir,name),buf=fs.readFileSync(p);
  if(buf.length!==size)throw new Error("size_mismatch:"+name);
...
const bootstrap=E.bootstrap(metrics,10000,17017);
const out={...metrics,bootstrap,execution_metadata:{
  engine_implementation:"research/code/engine-h-v0.1.js",
...
fs.writeFileSync("research/results/EXP-017-development-summary-v0.1.json",JSON.stringify(out,null,2)+"\n");
fs.writeFileSync("research/results/EXP-017-development-setups-v0.1.jsonl",metrics.setups.map(x=>JSON.stringify(x)).join("\n")+"\n");
fs.writeFileSync("research/results/EXP-017-development-trades-v0.1.jsonl",metrics.trades.map(x=>JSON.stringify(x)).join("\n")+"\n");
console.log("EXP017_DEVELOPMENT_SUMMARY",JSON.stringify({
