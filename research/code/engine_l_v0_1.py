...
def find_micro_entry(
    ...
    if not math.isfinite(v5) or v5<=0:
        return {"status":"invalid_v5","resolved_ts":decision_end,"pulled_back":False,"resumption_confirmed":False}

    times=df1["datetime"]
    start=int(times.searchsorted(decision_end,side="left"))
    if start>=len(df1):
        return {"status":"no_m1_after_decision","resolved_ts":decision_end,"pulled_back":False,"resumption_confirmed":False}
    ...
    for pos in range(start,max_scan):
        ...
        last_seen_ts=ts
        if ts>=cutoff:
            return {"status":"expired_session","resolved_ts":cutoff,"pulled_back":pulled,"resumption_confirmed":False}
        if prev_ts is not None and ts-prev_ts>pd.Timedelta(minutes=MAX_NEXT_ENTRY_GAP_MINUTES):
            return {"status":"cancel_gap","resolved_ts":ts,"pulled_back":pulled,"resumption_confirmed":False}
        prev_ts=ts
        ...
        if direction=="long" and low<=old_invalidation:
            return {"status":"cancel_old_invalidation","resolved_ts":ts,"pulled_back":pulled,"resumption_confirmed":False}
        ...
        if entry_pos>=max_scan or entry_pos>=len(df1):
            return {"status":"expired_before_next_open","resolved_ts":ts+pd.Timedelta(minutes=1),"pulled_back":True,"resumption_confirmed":True}
        entry_row=df1.iloc[entry_pos]
        entry_ts=entry_row["datetime"]
        if entry_ts>=cutoff:
            return {"status":"expired_session","resolved_ts":cutoff,"pulled_back":True,"resumption_confirmed":True}
        expected=ts+pd.Timedelta(minutes=1)
        if entry_ts-expected>pd.Timedelta(minutes=MAX_NEXT_ENTRY_GAP_MINUTES):
            return {"status":"cancel_entry_gap","resolved_ts":entry_ts,"pulled_back":True,"resumption_confirmed":True}

        entry=float(entry_row["open"])
        stop=(seen_low-tick) if direction=="long" else (seen_high+tick)
        if direction=="long" and not stop<entry:
            return {"status":"invalid_fresh_stop_geometry","resolved_ts":entry_ts,"pulled_back":True,"resumption_confirmed":True}
        ...
        t40=econ.get("T40")
        if not t40:
            return {"status":"no_t40_economics","resolved_ts":entry_ts,"pulled_back":True,"resumption_confirmed":True}
        if not t40.get("execution_admissible_pre_probability",False):
            return {
                "status":"triggered_but_economically_rejected",
                "resolved_ts":entry_ts,
                "pulled_back":True,
    ...
    return {
        "status":"expired_no_entry" if pulled else "expired_no_pullback",
        "resolved_ts":last_seen_ts+pd.Timedelta(minutes=1),
        "pulled_back":pulled,
        "resumption_confirmed":False,
    }
