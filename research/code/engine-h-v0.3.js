/* Engine H v0.3 frozen implementation.
 * Governed by strategies/engine-h-range-raid-preexisting-fvg-reversal/SPEC-v0.3.md
 */
(function(root){
"use strict";
...
function runEngineH(months,splitStart,splitEnd){
  ...
  function newSetup(dir,b,loc,range,m){
    const seq=++setupSeq;
    const s={id:"H3-S"+seq,seq,dir,sweepTs:b.closeTs,sweepStart:b.startTs,sweepHigh:b.h,sweepLow:b.l,
  ...
  function postRaidInvalidationPivot(s,m){
    const arr=s.dir==="SHORT"?pivH:pivL;
    for(let i=arr.length-1;i>=0;i--){
      const p=arr[i];
      if(p.confirmOrd>m.activeOrd)continue;
      if(p.centerOrd>=m.activeOrd)continue;
      if(p.centerTs<s.sweepTs)break;
      return p;
    }
    return null;
  }
  ...
  function evaluateEntryAdmission(s,m){
    const entry=m.o;
    const p=s.stopPivot;
    const stop=s.dir==="SHORT"?p.price+1:p.price-1;
    if((s.dir==="SHORT"&&!(entry<stop))||(s.dir==="LONG"&&!(stop<entry))){
      reject(s,"invalid_postraid_stop_geometry",m.ts);return null;
    }
    const stopDist=Math.abs(entry-stop);
    if(stopDist>4000){reject(s,"structural_risk_above_40",m.ts);return null;}
    const target=s.dir==="LONG"?entry+4000:entry-4000;
    s.entry=entry;s.stop=stop;s.target=target;s.stopDist=stopDist;s.targetDist=4000;s.grossStopUSD=stopDist*TICK_USD;
    s.oppositeRangeDelta=s.dir==="SHORT"?entry-s.rangeLow:s.rangeHigh-entry;
    return s;
  }
  ...
  function fillTradeAtOpen(s,m){
    const t={id:"H3-T"+(++tradeSeq),setupId:s.id,dir:s.dir,entry:s.entry,stop:s.stop,target:s.target,stopDist:s.stopDist,targetDist:4000,
      ...
      labels:{3000:null,4000:null,5000:null,7000:null,10000:null},labelTimes:{},locationFvgAge:s.locationFvgAge,locationDistance:s.locationDistance,
      oppositeRangeDelta:s.oppositeRangeDelta,stopPivotPrice:s.stopPivot.price,stopPivotCenterTs:s.stopPivot.centerTs,sweepTs:s.sweepTs};
  ...
  function processActual(t,m){
    ...
    if(target){t.actualMFE=Math.min(Math.max(t.actualMFE,4000),4000);closeTrade(t,"exit_t40",t.target,m.ts+MIN);return;}
  ...
  function processPendingMss(m){
    for(const s of pending){
      ...
      if(hit){
        s.mssOrd=m.activeOrd;s.mssTs=ct;
        const p=postRaidInvalidationPivot(s,m);
        if(!p){reject(s,"no_postraid_invalidation_pivot",ct);continue;}
        s.stopPivot=p;s.entryOrd=m.activeOrd+1;s.state="entry_next";
  ...
  function create5Fvg(b){
    ...
    if(c.l>a.h)fvgs.push({id:"H3-F"+(++fvgSeq),dir:"BULL",lower:a.h,upper:c.l,createOrd:c.activeOrd,createTs:c.closeTs,mitigated:false,expired:false});
    if(c.h<a.l)fvgs.push({id:"H3-F"+(++fvgSeq),dir:"BEAR",lower:c.h,upper:a.l,createOrd:c.activeOrd,createTs:c.closeTs,mitigated:false,expired:false});
  }
  function on5Close(b,m){
    if(!b.active)return;
    if(inSplit(b.closeTs))counts.active_5m++;
    if(!snapRange){create5Fvg(b);return;}
    if(snapRange.zero){if(inSplit(b.closeTs)){const s={id:"H3-S"+(++setupSeq),sweepTs:b.closeTs,state:"done",terminal:"range_zero_width"};setups.push(s);}create5Fvg(b);return;}
    ...
    if(qBear||qBull){
      ...
      if(qBear&&qBull){
        const s={id:"H3-S"+(++setupSeq),sweepTs:b.closeTs,state:"done",terminal:"dual_sided_raid_ambiguous"};setups.push(s);
      }else{
        ...
        if(!inWin){const s={id:"H3-S"+(++setupSeq),dir,sweepTs:b.closeTs,state:"done",terminal:"outside_setup_window"};setups.push(s);}
        else if(openTrade){const s={id:"H3-S"+(++setupSeq),dir,sweepTs:b.closeTs,state:"done",terminal:"suppressed_one_open"};setups.push(s);}
        else{if(inSplit(b.closeTs))counts.in_window_raids++;newSetup(dir,b,loc,snapRange,m);}
      }
    }else if((bh&&touchBear)||(bl&&touchBull)){
      const s={id:"H3-S"+(++setupSeq),sweepTs:b.closeTs,state:"done",terminal:"raid_without_rejection_close"};setups.push(s);
    }else if((bh||bl)&&!snapBear&&!snapBull){
      const s={id:"H3-S"+(++setupSeq),sweepTs:b.closeTs,state:"done",terminal:"no_external_preexisting_fvg"};setups.push(s);
    }
  ...
  return{
    ...
    location_fvg_age_active5:dist(tt.map(t=>t.locationFvgAge)),location_distance_ticks:dist(tt.map(t=>t.locationDistance)),opposite_range_delta_ticks:dist(tt.map(t=>t.oppositeRangeDelta)),direction,
...
function selfTest(){
  ...
  assert(500*TICK_USD===5,"primary_cost_exact");
  return{ok:true,tests:["tick","offgrid","gross_risk_40","primary_cost_5"]};
}
root.EngineHv03={runEngineH,bootstrap,selfTest,parseTick};
})(globalThis);
