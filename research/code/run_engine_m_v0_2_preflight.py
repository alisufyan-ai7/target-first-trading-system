#!/usr/bin/env python3
"""EXP-028 Engine M v0.2 zero-outcome signal-first/deployability preflight."""
...
from engine_m_v0_2 import (
    safe_deployability_overlay,
    self_tests_engine_m_v02,
)
...
CACHE=Path("/tmp/engine-m-v02-preflight")
...
def market_preflight(symbol:str,df1:pd.DataFrame,usd_jpy_df:pd.DataFrame|None=None)->dict:
    ...
    for i in range(len(bars["m15"])):
        ...
        # v0.2 signal layer: a causal filled setup with valid entry/stop/target geometry
        # remains a signal even if old USD40-equivalent sizing rejected it.
        ...
        if overlay.get("reason") in ("invalid_geometry","value_or_notional_unavailable","target_geometry_unavailable","invalid_stop_loss_per_lot"):
            continue
...
def main():
    ...
    per_market_gate={
        s:(
            markets[s]["filled_signal_paths"]>=50
    ...
    gate={
        ...
        "total_signal_paths_ge_600":total_signals>=600,
    ...
    passed=bool(
        ...
        and gate["total_signal_paths_ge_600"]
    ...
    result={
        ...
        "stage":"zero_outcome_signal_first_deployability_preflight",
        "target_outcomes_calculated":False,
        "pnl_outcomes_calculated":False,
    ...
    out=OUT/"EXP-028-signal-first-preflight-v0.2.json"
