...
function verifyCsv(name,buf,expectedRows){
  ...
  for(let i=1;i<lines.length;i++){
    ...
    assert(o>0&&h>=o&&h>=c&&l<=o&&l<=c&&h>=l,"bad_ohlc:"+name+":"+i);
  }
...
const dir=path.resolve("data/engine-i-v01");
...
const devRunner=fs.readFileSync("research/code/run-engine-i-development.js","utf8");
const devWorkflow=fs.readFileSync(".github/workflows/exp020-engine-i-development.yml","utf8");
...
assert(!/EngineHv0|EngineGv0/.test(engineText),"cross_engine_export_contamination");

const out={
  experiment:"EXP-020",
  engine:"Engine I v0.1",
  ...
  harness_checks:{
    ...
    strict_context_path_present:true,
...
fs.writeFileSync("research/results/EXP-020-preflight-v0.1.json",JSON.stringify(out,null,2)+"\n");
console.log("EXP020_PREFLIGHT_OK",JSON.stringify({files:files.length,totalRows,tests:tests.tests.length,outcomes:false}));
