/* Engine H v0.1 frozen implementation.
 * Governed by strategies/engine-h-range-raid-preexisting-fvg-reversal/SPEC-v0.1.md
 */
(function(root){
"use strict";
...
function runEngineH(months,splitStart,splitEnd){
  ...
  function newSetup(dir,b,loc,range,m){
    const s={id:"H-S"+(++setupSeq),dir,sweepTs:b.closeTs,sweepStart:b.startTs,sweepHigh:b.h,sweepLow:b.l,
      ...
      createdOrd:m.activeOrd,state:"mss",mssSeen:0,entrySeen:0,terminal:null};
  ...
  function evalSignalBar(s,m){
    const ord=m.activeOrd;
    if(ord<s.mssOrd||ord>s.mssOrd+2)return false;
    if(ord<20){reject(s,"displacement_baseline_unavailable_20",m.ts+MIN);return true;}
    let sum=0;for(let i=ord-20;i<ord;i++)sum+=Math.abs(activeM1[i].c-activeM1[i].o);
    const body=Math.abs(m.c-m.o);
    const disp=s.dir==="SHORT"?(m.c<m.o&&m.c<s.pivot.price&&40*body>=3*sum):(m.c>m.o&&m.c>s.pivot.price&&40*body>=3*sum);
    if(disp)s.sawDisp=true;
    let fvg=null;
    if(disp&&ord>=2){
      const a=activeM1[ord-2];
      if(s.dir==="LONG"&&m.l>a.h)fvg={lo:a.h,hi:m.l};
      if(s.dir==="SHORT"&&m.h<a.l)fvg={lo:m.h,hi:a.l};
    }
    if(fvg){
      s.dispOrd=ord;s.dispTs=m.ts+MIN;s.body=body;s.body20sum=sum;s.entryFvgLo=fvg.lo;s.entryFvgHi=fvg.hi;s.entryFvgTs=m.ts+MIN;
      const num=fvg.lo+fvg.hi;
      s.entry=s.dir==="LONG"?Math.floor((num+1)/2):Math.floor(num/2);
      s.stop=s.dir==="SHORT"?s.sweepHigh+1:s.sweepLow-1;
      s.target=s.dir==="SHORT"?s.rangeLow:s.rangeHigh;
      if((s.dir==="LONG"&&s.stop>=s.entry)||(s.dir==="SHORT"&&s.stop<=s.entry)){reject(s,"invalid_stop_geometry",m.ts+MIN);return true;}
      s.stopDist=Math.abs(s.entry-s.stop);s.grossStopUSD=s.stopDist*TICK_USD;
      if(s.stopDist>4000){reject(s,"structural_risk_above_40",m.ts+MIN);return true;}
      if((s.dir==="SHORT"&&!(s.target<s.entry&&s.entry<s.stop))||(s.dir==="LONG"&&!(s.stop<s.entry&&s.entry<s.target))){reject(s,"invalid_target_geometry",m.ts+MIN);return true;}
      s.targetDist=Math.abs(s.target-s.entry);
      if(s.targetDist<3000){reject(s,"insufficient_target_room_3xau",m.ts+MIN);return true;}
      if(s.targetDist<2*s.stopDist){reject(s,"reward_risk_below_2",m.ts+MIN);return true;}
      s.state="entry";s.entrySeen=0;return true;
    }
    if(ord===s.mssOrd+2){reject(s,s.sawDisp?"no_entry_fvg_by_mss_plus_2":"displacement_timeout_mss_plus_2",m.ts+MIN);return true;}
    return false;
  }
  ...
  function fillTrade(s,m){
    if(openTrade){reject(s,"suppressed_one_open",m.ts+MIN);return null;}
    const t={id:"H-T"+(++tradeSeq),setupId:s.id,dir:s.dir,entry:s.entry,stop:s.stop,target:s.target,stopDist:s.stopDist,targetDist:s.targetDist,
      fillTs:m.ts+MIN,fillOrd:m.activeOrd,horizonCount:1,actualMFE:0,actualMAE:0,potentialMFE:0,potentialMAE:0,closed:false,shadowDone:false,
    ...
    if(stopHit){
      t.actualMAE=t.stopDist;t.potentialMAE=t.stopDist;for(const k of Object.keys(t.labels))t.labels[k]="stop_before_target";t.shadowDone=true;
      closeTrade(t,"exit_stop_fill_bar",t.stop,m.ts+MIN);
    }
    return t;
  }
  ...
  function processShadow(t,m){
    ...
    for(const ks of Object.keys(t.labels)){if(t.labels[ks]!==null)continue;const d=Number(ks),px=t.dir==="LONG"?t.entry+d:t.entry-d;const hit=t.dir==="LONG"?m.h>=px:m.l<=px;if(hit){t.labels[ks]="target_before_stop";t.labelTimes[ks]=(m.ts+MIN)-t.fillTs;}}
  ...
  function processPending(m){
    for(const s of pending){
      ...
      if(openTrade){reject(s,"suppressed_one_open",ct);continue;}
      if(!before1800(ct)){
        if(s.state==="mss")reject(s,"setup_window_closed_before_mss",ct);
        else if(s.state==="signal")reject(s,"setup_window_closed_before_entry_fvg",ct);
        else if(s.state==="entry")reject(s,"entry_window_closed_1800",ct);
        continue;
      }
      if(s.state==="mss"){
        s.mssSeen++;const hit=s.dir==="SHORT"?m.c<s.pivot.price:m.c>s.pivot.price;
        if(hit){s.mssOrd=m.activeOrd;s.mssTs=ct;s.state="signal";s.sawDisp=false;evalSignalBar(s,m);}
        else if(s.mssSeen>=10)reject(s,"mss_timeout_10",ct);
      }else if(s.state==="signal")evalSignalBar(s,m);
      else if(s.state==="entry"){
        s.entrySeen++;
        const entry=s.dir==="LONG"?m.l<=s.entry:m.h>=s.entry;
        if(entry){fillTrade(s,m);continue;}
        const stop=s.dir==="LONG"?m.l<=s.stop:m.h>=s.stop;
        const target=s.dir==="LONG"?m.h>=s.target:m.l<=s.target;
        if(stop){reject(s,"preentry_structural_invalidation",ct);continue;}
        if(target){reject(s,"preentry_target_reached",ct);continue;}
        if(s.entrySeen>=10)reject(s,"entry_timeout_10",ct);
      }
    }
  ...
  function create5Fvg(b){
    ...
    if(c.l>a.h)fvgs.push({id:"H-F"+(++fvgSeq),dir:"BULL",lower:a.h,upper:c.l,createOrd:c.activeOrd,createTs:c.closeTs,mitigated:false,expired:false});
    if(c.h<a.l)fvgs.push({id:"H-F"+(++fvgSeq),dir:"BEAR",lower:c.h,upper:a.l,createOrd:c.activeOrd,createTs:c.closeTs,mitigated:false,expired:false});
  }
  function on5Close(b,m){
    if(!b.active)return;
    if(inSplit(b.closeTs))counts.active_5m++;
    if(!snapRange){create5Fvg(b);return;}
    if(snapRange.zero){if(inSplit(b.closeTs)){const s={id:"H-S"+(++setupSeq),sweepTs:b.closeTs,state:"done",terminal:"range_zero_width"};setups.push(s);}create5Fvg(b);return;}
    ...
    if(qBear||qBull){
      ...
      if(qBear&&qBull){
        const s={id:"H-S"+(++setupSeq),sweepTs:b.closeTs,state:"done",terminal:"dual_sided_raid_ambiguous"};setups.push(s);
      }else{
        ...
        if(!inWin){const s={id:"H-S"+(++setupSeq),dir,sweepTs:b.closeTs,state:"done",terminal:"outside_setup_window"};setups.push(s);}
        else if(openTrade){const s={id:"H-S"+(++setupSeq),dir,sweepTs:b.closeTs,state:"done",terminal:"suppressed_one_open"};setups.push(s);}
        else{if(inSplit(b.closeTs))counts.in_window_raids++;newSetup(dir,b,loc,snapRange,m);}
      }
    }else if((bh&&touchBear)||(bl&&touchBull)){
      const s={id:"H-S"+(++setupSeq),sweepTs:b.closeTs,state:"done",terminal:"raid_without_rejection_close"};setups.push(s);
    }else if((bh||bl)&&!snapBear&&!snapBull){
      const s={id:"H-S"+(++setupSeq),sweepTs:b.closeTs,state:"done",terminal:"no_external_preexisting_fvg"};setups.push(s);
    }
  ...
  const daily={};for(const d of dates)daily[d]=0;for(const t of tt){const d=utcDate(t.fillTs-1);if(daily[d]!==undefined)daily[d]+=t.netUSD500;}
...
function bootstrap(metrics,reps=10000,seed=17017){
  ...
  for(const t of metrics.trades){const d=utcDate(t.fillTs-1);if(by.has(d))by.get(d).push(t.netUSD500);}
...
function selfTest(){
  ...
  assert(Math.floor((1001+1002+1)/2)===1002,"long_mid");assert(Math.floor((1001+1002)/2)===1001,"short_mid");
  assert(40*150>=3*2000&&!(40*149>=3*2000),"disp_exact");assert(3000>=2*1500&&!(2999>=2*1500),"rr_exact");
  return{ok:true,tests:["tick","offgrid","midpoint","displacement_1.50","reward_risk_2.0"]};
}
root.EngineHv01={runEngineH,bootstrap,selfTest,parseTick};
})(globalThis);
