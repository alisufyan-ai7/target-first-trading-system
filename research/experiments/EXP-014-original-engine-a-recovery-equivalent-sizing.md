...
## Part A — A1 reconstruction checkpoint

**Status: FAIL — NOT THE RECOVERED EXP-002 IMPLEMENTATION**

Frozen A1 specification:

- `strategies/engine-a-liquidity-mss-fvg/SPEC-v0.1-recovery-A1.md`;
- final pre-outcome A1 spec commit: `448aef86af7e7d5227c4e4b9405f9d0c1f8cae03`.

No A1 mechanics were changed after outcome inspection.

### Data actually evaluated

A1 used the documented March 1 through August 20, 2026 XAUUSD one-minute bid-series window through a public Dukascopy-derived mirror used only as external research data.

Audit for the frozen window:

- one-minute rows: **230,813**, exactly matching the EXP-002 recorded row count;
- first timestamp: 2026-03-01 00:00 UTC;
- last timestamp: 2026-08-20 23:58 UTC;
- duplicate timestamps: **0**;
- invalid OHLC rows: **0**;
- UTC-aligned complete 5m bars used by A1: **46,157**.

External mirror file blob SHAs used:
...
Limitation: the mirror reproduces the recorded EXP-002 row count exactly, but it has not yet been checksum-matched against the original local EXP-002 download. It is therefore recorded as Dukascopy-derived research data, not assumed to prove byte-for-byte identity with the unpublished original dataset.

### A1 signal funnel

- qualifying 5m sweep events: **2,346**;
- with eligible internal pivot: **2,340**;
- reaching MSS: **1,206**;
- passing displacement: **531**;
- reaching qualifying FVG: **424**;
- midpoint fills before one-open suppression: **258**;
- sweeps suppressed because an A1 trade was already open: **2**;
- accepted trades: **256**.

### Complete A1 benchmark vector

| Metric | EXP-002 benchmark | Frozen acceptance band | A1 | Result |
|---|---:|---:|---:|---|
| Overall trades | ~372 | 335–409 | **256** | FAIL |
| Development trades | ~185 | 163–207 | **129** | FAIL |
| Holdout trades | ~187 | 165–209 | **127** | FAIL |
| Overall T5 | 29.6% | 26.6–32.6% | **11.72%** | FAIL |
| Holdout T2 | 48.7% | 44.7–52.7% | **22.83%** | FAIL |
| Holdout T3 | 41.7% | 37.7–45.7% | **18.90%** | FAIL |
| Holdout T4 | 31.6% | 27.6–35.6% | **14.17%** | FAIL |
| Holdout T5 | 27.3% | 24.3–30.3% | **11.02%** | FAIL |
| Avg structural risk | ~1.02 Gold | 0.867–1.173 | **0.825** | FAIL |
| Median MFE | ~3.30 Gold | 2.805–3.795 | **0.00** | FAIL |
| Overall simplified expectancy | +0.79 | +0.514 to +1.067 | **-0.103** | FAIL |
| Development expectancy | +0.85 | +0.553 to +1.148 | **-0.095** | FAIL |
| Holdout expectancy | +0.72 | +0.468 to +0.972 | **-0.111** | FAIL |

Additional outcome counts:

- T5 winners: **30**;
- structural-stop outcomes: **226**;
- timeouts: **0**.
...
- trades: 129;
- T5: 12.40%;
- expectancy: -0.095 Gold/trade.

Holdout:

- trades: 127;
- T2/T3/T4/T5: 22.83% / 18.90% / 14.17% / 11.02%;
- expectancy: -0.111 Gold/trade.

### Classification

**FAIL**, not NEAR MATCH.

A1 misses every frozen acceptance dimension. In particular:

1. accepted trade count is about 31% below the ~372 benchmark;
2. holdout target-first rates are roughly half the recorded EXP-002 ladder;
3. simplified expectancy has the wrong sign in both development and holdout;
4. median MFE collapses to zero under the A1 stop/fill lifecycle;
5. average risk is somewhat too tight as well.

The zero median MFE plus 226/256 structural-stop outcomes indicates that the A1 tight-FVG-stop hypothesis interacts very poorly with conservative midpoint-fill bar handling. This is a diagnostic observation, not permission to relax the project's conservative same-bar rule.

### Required next step before A2

Do **not** change multiple parameters.

First decompose the A1 structural-stop outcomes into:

- stop touched on the midpoint fill bar;
- stop touched only after the fill bar;
